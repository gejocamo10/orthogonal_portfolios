...
for (i in 1:n_etfs){
  ...
  df <- fit_mvt(data_list[[i]], nu = "iterative")$nu
  data_sim_loc <- rmvt(n = t, sigma = covariance_list[[i]]*((df-2)/df),
                       df = df) + matrix(rep(media_list[[i]],t), ncol = n, byrow = TRUE)
...
for (h in h_vector){ #h
  ...
  for (i in 1:n_etfs){ #etf
    ...
    write.csv(etf_sim_rets[[i]], file =
                paste0("data_simulation/retornos_t_", etfs[i], "_", h,
  ...
  write.csv(round(results_etf * 100, 3),
            file = paste("data_simulation/retornos_t_h_", h, ".csv", sep = ""),
            row.names = FALSE)
  count <- count + 1

  }# cada h

save.image(file = "environment_t.RData")