...
for (i in 1:n_etfs){
  ...
  data_sim_loc <- simulate_wcopula(num = t, data_list[[i]])
...
for (h in h_vector){ #h
  ...
  for (i in 1:n_etfs){ #etf
    ...
    write.csv(etf_sim_rets[[i]], file =
                paste0("data_simulation/retornos_copula_", etfs[i], "_", h,
  ...
  write.csv(round(results_etf * 100, 3),
            file = paste("data_simulation/retornos_copula_h_", h, ".csv", sep = ""),
            row.names = FALSE)
  count <- count + 1

  }# cada h

save.image(file = "environment_copula.RData")